    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-8">
            ...
            <div class="flex items-center space-x-4">
                ...
                <div>
                    <h2 class="text-3xl font-bold text-gray-900">Chỉnh sửa người dùng</h2>
                    <p class="text-gray-500 mt-1">{{ $user->name }} <span class="text-gray-400">•</span> <span
                            class="text-sm">{{ $user->email }}</span></p>
                </div>
            </div>
        </div>

        <!-- Form -->
        <form method="POST" action="{{ route('admin.users.update', $user->_id) }}">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-8 mb-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" id="user_code" name="user_code" value="{{ old('user_code', $user->user_code) }}"
                            required
                    ...
                    <div>
                        ...
                        <p class="mt-1.5 text-xs text-gray-500">💡 Chỉ nhập nếu muốn thay đổi mật khẩu</p>
                        @error('password')
                            <p class="mt-2 text-sm text-red-600 flex items-center">
                                <svg class="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd"
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-8 mb-6">
                <div class="flex items-center mb-6 pb-4 border-b border-gray-200">
                    ...
                    <h3 class="text-lg font-semibold text-gray-900">Vai trò & Phân quyền</h3>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div>
                        ...
                        <select id="role" name="role" required
                            ...
                            <option value="content_manager" {{ old('role', $user->role) == 'content_manager' ? 'selected' : '' }}>✍️ Content Manager</option>
                    ...
                    <div>
                        ...
                        <select id="status" name="status" required
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition bg-white">
                            <option value="active" {{ old('status', $user->status) == 'active' ? 'selected' : '' }}>✅ Hoạt
                                động</option>
                            <option value="pending" {{ old('status', $user->status) == 'pending' ? 'selected' : '' }}>⏳ Chờ
                                duyệt</option>
                            <option value="inactive" {{ old('status', $user->status) == 'inactive' ? 'selected' : '' }}>🔒 Đã
                                khóa</option>
                        </select>
                        @error('status')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <select id="department_id" name="department_id"
                            ...
                            @foreach($departments as $dept)
                                <option value="{{ $dept->_id }}" {{ old('department_id', $user->department_id) == $dept->_id ? 'selected' : '' }}>
                                    {{ $dept->name }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl border border-gray-200 p-6 mb-6">
                <div class="flex items-center space-x-6 text-sm text-gray-600">
                    ...
                    <div class="flex items-center">
                        ...
                        <span class="ml-2">{{ $user->updated_at?->format('d/m/Y H:i') ?? 'N/A' }}</span>
                    </div>
                </div>
            </div>
